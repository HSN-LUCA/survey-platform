  ...
  const loadSurvey = async () => {
    try {
      ...
      if (survey.questions && survey.questions.length > 0) {
        const loadedQuestions = survey.questions.map((q: any) => ({
          id: q.id,
          type: q.type,
      ...
      if (responsesResponse.ok) {
        ...
        setHasResponses((responsesData.responses || []).length > 0);
      }
    } catch (err) {
      ...
      <div className={`max-w-6xl mx-auto ${isRTL ? 'rtl' : 'ltr'}`}>
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-yellow-600 to-yellow-700 bg-clip-text text-transparent mb-2">
            {isRTL ? 'تعديل الاستبيان' : 'Edit Survey'}
          </h1>
          <p className="text-gray-600">
            {isRTL ? 'قم بتعديل تفاصيل الاستبيان والأسئلة' : 'Modify survey details and questions'}
          </p>
          {hasResponses && (
          ...
          <div className="flex gap-4">
            ...
            >
              {loading ? (
                ...
                isRTL ? 'تحديث الاستبيان' : 'Update Survey'
              )}
            </button>
            <button
              type="button"
